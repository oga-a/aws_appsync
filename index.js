const AppSync = {
    "graphqlEndpoint": "https://*****.amazonaws.com/graphql",
    "apiKey": "*****"
};
...
(function ($) {
        ...
        $.ajax({
            ...
            for (let i = 0; i < data.data.listUsers.length; i++) {
                ...
                for (let todo of user.todos) {
                    tr += `
                        <li data-userid="${user.id}" data-todoid="${todo.id}">
    ...
    const deleteTodo = (todoUserId, todoId) => {
        const param = JSON.stringify({
            'query': `mutation {
                deleteTodo(userId: "${todoUserId}", id: "${todoId}") {
                    id
                }
            }`
        });
    ...
    $(function () {
        ...
        $('#todoList')
            ...
            .on('click', 'li button.mini.alert', function () {
                deleteTodo($(this).closest('li').data('userid'), $(this).closest('li').data('todoid'));
            });
